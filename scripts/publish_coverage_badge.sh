...
# editing a file. This repo carried no coverage badge and no enforced floor at
# all before alpha-engine-config-I10432.
#
...
# own.
#
# Runs from .github/workflows/test.yml on pushes to main only (one job, one
# leg — no matrix to race). The `badges` branch is seeded on the remote
# before the PR that adds this script is opened, so the contents API is
# enough and no branch is ever created here.
set -euo pipefail
...
# The same measurement the gate uses — coverage.py's own totals over
# [tool.coverage.run] source in pyproject.toml, not a figure re-derived from
# parsed stdout.
